...
TEST_CASE("async queue", "[async]" ){
	...
	SECTION("base3") {
		...
		io.post([p, num, &io, errinfo]() {
			...
			p->apply(io, "key1", ara::timer_val::max_time, "thread 1 apply 1").then( [num, errinfo](const boost::system::error_code & ec, ara::async_token token) {
		...
		io.post([p, num, &io, errinfo]() {
			...
			p->apply(io, "key1", ara::timer_val(0, 1), "thread 2 apply 1").then( [num, &io, p, errinfo](const boost::system::error_code & ec, ara::async_token token) {
				...
				if (token != nullptr) {
					...
					io.post([p, num, &io, errinfo]() {
						p->apply(io, "key1", ara::timer_val(0, 1), "thread 2 apply 2").then( [num, errinfo](const boost::system::error_code & ec, ara::async_token token) {
							ara::glog		g(ara::log::info);
							if (token == nullptr) {
								errinfo->set_error(ara::printf<std::string>("T2 should got token after T1 release it. But T2 can not got it now:%v", ec.message()));
							} else {
