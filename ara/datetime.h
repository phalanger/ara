...
#define  DEFAULT_DATETIME_FORMAT_L		L"%Y-%m-%d %H:%M:%S"
	...
	public:
		...
		const date_time_imp&  set(int year, int month, int day,
			...
			traits::set_time(time_, internal::standard_time_op::mktime(year, month, day, hour, minute, second, isdst));
		}
		...
		const date_time_imp & set(const struct tm & data) {
			traits::set_time(time_, internal::standard_time_op::mktime(data));
			return *this;
		}
		...
		inline void get(struct tm & data) const {
			internal::standard_time_op::local_time(traits::get_time(time_), data);
		}
		inline void get_gmt(struct tm & data) const {
			internal::standard_time_op::gmt_time(traits::get_time(time_), data);
		}
		...
		std::wstring 	wformat(const wchar_t * pattern = nullptr) const {
			return internal::standard_time_op::local_format<wchar_t, std::wstring>(traits::get_time(time_), pattern == nullptr ? DEFAULT_DATETIME_FORMAT_L : pattern);
		}
		std::string 	format(const char * pattern = nullptr) const {
			return internal::standard_time_op::local_format<char, std::string>(traits::get_time(time_), pattern == nullptr ? DEFAULT_DATETIME_FORMAT : pattern);
		}
		std::wstring 	wformat_gmt(const wchar_t * pattern = nullptr) const {
			return internal::standard_time_op::gmt_format<wchar_t, std::wstring>(traits::get_time(time_), pattern == nullptr ? DEFAULT_DATETIME_FORMAT_L : pattern);
		}
		std::string 	format_gmt(const char * pattern = nullptr) const {
			return internal::standard_time_op::gmt_format<char, std::string>(traits::get_time(time_), pattern == nullptr ? DEFAULT_DATETIME_FORMAT : pattern);
		}

		//! Parse the DateTime string
		bool		parse(const char * date, const char * pattern) {
			struct tm info = { 0 };
			bool bo = internal::standard_time_op::parse(info, pattern == nullptr ? DEFAULT_DATETIME_FORMAT : pattern, date);
			if (bo)
				set(info);
			return bo;
		}
		bool		parse(const wchar_t * date, const wchar_t * pattern) {
			struct tm info = { 0 };
			bool bo = internal::standard_time_op::parse(info, pattern == nullptr ? DEFAULT_DATETIME_FORMAT_L : pattern, date);
			if (bo)
				set(info);
			return bo;
		}
	...
	using date_time = date_time_imp<time_t, internal::standard_time_traits>;
